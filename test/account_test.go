...
func TestAccountDetail(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		if testCase.ExpectedError != "" && !strings.Contains(body, testCase.ExpectedError) {
			t.Errorf("TestAccountDetail #%v: expected error %v but got %v", i+1, testCase.ExpectedError, body)
		}
		if testCase.ExpectedReponse != "" && body != testCase.ExpectedReponse {
			t.Errorf("TestAccountDetail #%v: expected account_detail %v but got %v", i+1, testCase.ExpectedReponse, body)
		}
